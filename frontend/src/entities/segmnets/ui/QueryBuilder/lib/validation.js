...
import {deepEqual, immutableEqual, logger} from './stuff';

const {getOperatorsForField, getWidgetForFieldOp, getNewValueForFieldOp} = Utils.RuleUtils;
const {defaultOperatorOptions} = Utils.DefaultUtils;
const {getItemInListValues} = Utils.ListUtils;
const {fixPathsInTree} = Utils.TreeUtils;
...
export const validateAndFixTree = (
    newTree,
    _oldTree,
    newConfig,
    oldConfig,
...
function validateGroup(item, path, itemId, meta, c) {
    ...
    if (removeEmptyGroups) children = children.filter((currentChild) => currentChild != undefined);
...
function validateRule(item, path, itemId, meta, c) {
    ...
    if (!availOps) {
        ...
        if (operator == 'is_empty' || operator == 'is_not_empty') {
            ...
            console.info(
                `Fixed operator ${properties.get('operator')} to ${operator} for ${field}`,
            );
            properties = properties.set('operator', operator);
        } else {
...
export const createValidationMemo = () => {
    ...
    return (config, tree, oldConfig = undefined, sanitizeTree = true) => {
        ...
        if (
            config.__configId === configId &&
            (immutableEqual(tree, originalTree) || immutableEqual(tree, validatedTree))
        ) {
            ...
            if (sanitizeTree === false) {
                validatedTree = validateAndFixTree(
                    tree,
                    null,
                    config,
                    oldConfig || config,
